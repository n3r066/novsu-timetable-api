...
Ответ — ТЕКСТОВАЯ вырезка из поста канала: шапка «СРЕДА · 23.09.2026 · 5 пар · 2 ДОТ»
и пары (номер, время, тип, предмет, препод, место/ДОТ). Никаких картинок:
фотки в группе липнут в галерею и эфемерность ломают. Данные — те же
state/last_parsed.json, что рисуют закреп, рендера ноль.
